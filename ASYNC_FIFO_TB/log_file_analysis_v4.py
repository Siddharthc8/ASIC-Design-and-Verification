...
# This code displays write and read side by side with common count column in the middle 
...
def print_data(data):
    ...
    print(f"{'=' * 70}")
    print(f"{'WRITE':^24} | {'READ':^24}")
    print(f"{'TIME(ns)':>10} {'DATA':>8} | {'COUNT':^7} | {'DATA':<8} {'TIME(ns)':<10}")
    print(f"{'-' * 70}")
    ...
    for i in range(max_len):
        ...
        # Format write side
        if write_entry:
            write_str = f"{write_entry['timestamp']:>10} {write_entry['value']:>8}"
        else:
            write_str = f"{'-':^11} {'-':^10}"

        # Format read side
        if read_entry:
            read_str = f"{read_entry['value']:<10} {read_entry['timestamp']:<10}"
        else:
            read_str = f"{'-':^11} {'-':^10}"

        print(f"{write_str} | {i:^7} | {read_str}")
